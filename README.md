...
3. Обработка выбора через switch-case
    - Команда 0: Завершение работы
    - Команда 1: Просмотр коробок (Производится проверка наличия коробок, последовательный вывод каждой из существующих коробок с помощью метода toString() из класса GeneralizedBox.)
    - Команда 2: Создание коробки с числом (Реализован бесконечный цикл до получения кооректного ввода, проверка осуществляется с помощью метода valIsNumber() из класса Validator, принеобходимости выводится соответствующее сообщение о некорректном вводе. При вводе коректного значения определяется тип введённого числа с помощью методов valIsInt() и  valIsDouble() из класса Validator и создаётся коробка соответствующего типа, которая добавляется в созданную коллекцию. Выводится сообщение об успешном создании.)
...
Для проверки корректности ввода в классе Validator реализованы методы valIsInt(String value) - проверка строки на соответствие целочисленному формату, valIsDouble(String value) - проверка строки на соответствие формату чисел с плавающей точкой, valIsNumber(String value) - проверка на числовой формат (целый или дробный).
